...
unsigned char generate_output(unsigned int eggs_in_the_box, unsigned int eggs_in_the_warehouse){
    ...
    */
    unsigned int output_byte = eggs_in_the_box;
    output_byte = output_byte << 2;
    PRINT("output generated by manager: %u\n", output_byte);
    unsigned int eggs_to_move = NUMBER_OF_EGGS_IN_THE_BOX - eggs_in_the_box < eggs_in_the_warehouse ?
                                NUMBER_OF_EGGS_IN_THE_BOX - eggs_in_the_box:
                                eggs_in_the_warehouse;
    PRINT("Eggs to move: %d\n", eggs_to_move);
    output_byte = output_byte | eggs_to_move;
    output_byte = output_byte << 3;
    PRINT("output generated by manager: %u\n", output_byte);
    // TODO: define what does order eggs mean!!

    unsigned char result = output_byte;
...
unsigned int count_eggs_in_the_warehouse(unsigned int byte_received){
    // read last 2 bits. Mask is 3.

    unsigned int eggs_in_the_warehouse = byte_received & 3;
    PRINT("Eggs in the warehouse: %d\n", eggs_in_the_warehouse);
    return eggs_in_the_warehouse;
...
char* read_input(int pipe_input_read, int pipe_input_write){
    ...
    char * msg_received = malloc(sizeof(char) * MAX_INFO_TO_SEND_SIZE);
...
void manage_input_output(int pid_input, int pid_output, int pipe_input_read, int pipe_input_write, int pipe_output_read, int pipe_output_write) {
    ...
    while (1){
         ...
         */
        unsigned char output = process_input(msg_received);
        free(msg_received);
        /* 4) Trigger the output_manager process and send information.
